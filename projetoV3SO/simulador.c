...
void *criarClientes(void *in, pthread_t* appthread)
{
   ...
   /* printf("Sala que se encontra é %s", (momento_sala ? "inicial" : "outra"));*/
    ...
    while (1) {
        ...
        if(valorSemSalaIni>0 && flagMessageSalaIni2 == 1) {
            ...
            if(flagMessageSalaIni2 == 0){ // a mensagem só aparece uma vez
                pthread_mutex_lock(&mensagensMonitor);
                int buffer_len = snprintf(buffer,BUFFER_SIZE, "%i*1",id);//meter em códigos para o monitor no conf. Neste exemplo é ficou na fila da sala inicial
        ...
        if(counterSalaIni == probalidadeDesistir) {
            pthread_mutex_lock(&mensagensMonitor);
            int buffer_len = snprintf(buffer,BUFFER_SIZE, "%i*2",id);//desistiu de entrar na sala inicial
            int n = write(data_socket, buffer, buffer_len);
            sleep(1);
            pthread_mutex_unlock(&mensagensMonitor);
            pthread_mutex_lock(&trincoVarGlobais);
    ...
    if (flagDesitiuSalaInicial == 0){// se não desistiu da fila entra na sala inicial
        ...
        flagMessageSalaIni2 = 0;
        pthread_mutex_lock(&mensagensMonitor);
        int buffer_len = snprintf(buffer,BUFFER_SIZE, "%i*3",id);//entrou na sala inicial
        int n = write(data_socket, buffer, buffer_len);
        sleep(1);
        pthread_mutex_unlock(&mensagensMonitor);
        ...
        pthread_mutex_lock(&mensagensMonitor);
        buffer_len = snprintf(buffer,BUFFER_SIZE, "%i*4",id);//saiu da sala inicial
 ...
 while (!acabou)
    {
        ...
        if(prox_sala == 0){//vai para a pista de dança
            while (1) {
                ...
                if(valorSemSalaDanca>0) {        
                    ...
                    if(flagMessageSalaDanca == 0){ // a mensagem só aparece uma vez
                        pthread_mutex_lock(&mensagensMonitor);
                        int buffer_len = snprintf(buffer,BUFFER_SIZE, "%i*5",id);//ficou na fila da pista de dança
                        int n = write(data_socket, buffer, buffer_len);
                        pthread_mutex_unlock(&mensagensMonitor);
                        pthread_mutex_lock(&trincoVarGlobais);
                ...
                if(counterSalaDanca == probalidadeDesistir) {
                    pthread_mutex_lock(&mensagensMonitor);
                    int buffer_len = snprintf(buffer,BUFFER_SIZE, "%i*6",id);//desitiu da fila da pista de dança
                    int n = write(data_socket, buffer, buffer_len);
                    pthread_mutex_unlock(&mensagensMonitor);
                    pthread_mutex_lock(&trincoVarGlobais);
            ...
            if (flagDesitiuSalaDanca == 0){// se não desistiu da fila entra na sala inicial
                flagMessageSalaDanca = 0;
                pthread_mutex_lock(&mensagensMonitor);
                int buffer_len = snprintf(buffer,BUFFER_SIZE, "%i*7",id);//entrou na pista de dança
                int n = write(data_socket, buffer, buffer_len);
                pthread_mutex_unlock(&mensagensMonitor);
                sleep(dados.tempo_med_sala_danca); // tempo a dançar
                sem_post(&sem_sala_ini);
                pthread_mutex_lock(&mensagensMonitor);
                buffer_len = snprintf(buffer,BUFFER_SIZE, "%i*8",id);//saiu da pista de dança
                n = write(data_socket, buffer, buffer_len);
                pthread_mutex_unlock(&mensagensMonitor);
            }
        ...
        if (prox_sala == 1) // Ira para o bar
        {
            ...
            int buffer_len = snprintf(buffer, BUFFER_SIZE, "%i*15",id);//entrou no bar
            int n = write(data_socket, buffer, buffer_len);
            sleep(1);
            pthread_mutex_unlock(&mensagensMonitor);
            pthread_mutex_lock(&trincoVarGlobais);
            ...
            if(beb < 26){//vai ser expulso por estar a fazer confusão e estar bêbado
                ...
                buffer_len = snprintf(buffer, BUFFER_SIZE, "%i*25",id);//foi expulso
                n = write(data_socket, buffer, buffer_len);
                sleep(1);
                pthread_mutex_unlock(&mensagensMonitor);
                pthread_mutex_lock(&trincoVarGlobais);
            ...
            pthread_mutex_lock(&mensagensMonitor); //trinco para enviar mensagem que saiu do bar
            buffer_len = snprintf(buffer, BUFFER_SIZE, "%i*16",id);//saiu do bar
            n = write(data_socket, buffer, buffer_len);
            sleep(1);
            pthread_mutex_unlock(&mensagensMonitor);
        ...
        { // IR A WC
            while (1)
            {
                ...
                else
                {
                    ...
                    {
                        pthread_mutex_lock(&mensagensMonitor);
                        int buffer_len = snprintf(buffer, BUFFER_SIZE, "%i*9",id);//ficou na fila da casa de banho
                        int n = write(data_socket, buffer, buffer_len);
                        sleep(1);
                        pthread_mutex_unlock(&mensagensMonitor);
                        pthread_mutex_lock(&trincoVarGlobais);
            ...
            {
                pthread_mutex_lock(&mensagensMonitor);
                int buffer_len = snprintf(buffer, BUFFER_SIZE, "%i*11",id);//entrou na casa de banho
                int n = write(data_socket, buffer, buffer_len);
                sleep(1);
                pthread_mutex_unlock(&mensagensMonitor);
                pthread_mutex_lock(&trincoVarGlobais);
                ...
                pthread_mutex_lock(&mensagensMonitor);
                buffer_len = snprintf(buffer, BUFFER_SIZE, "%i*12",id);//saiu da casa de baho
                n = write(data_socket, buffer, buffer_len);
                sleep(1);
                pthread_mutex_unlock(&mensagensMonitor);
            ...
            if (VIP) // IRA SALA VIP
                {
                    ...
                    pthread_mutex_lock(&mensagensMonitor);
                    int buffer_len = snprintf(buffer, BUFFER_SIZE, "%i*23",id);//entrou na sala VIP
                    int n = write(data_socket, buffer, buffer_len);
                    sleep(1);
                    pthread_mutex_unlock(&mensagensMonitor);
                    pthread_mutex_lock(&trincoVarGlobais);
                    ...
                    pthread_mutex_lock(&mensagensMonitor);
                    buffer_len = snprintf(buffer, BUFFER_SIZE, "%i*24",id);//saiu da sala VIP
                    n = write(data_socket, buffer, buffer_len);
                    sleep(1);
                    pthread_mutex_unlock(&mensagensMonitor);
            ...
            if (prox_sala == 4)//vai para a sala de bilhar
        {
            ...
            pthread_mutex_unlock(&trincoVarGlobais);
            pthread_mutex_lock(&mensagensMonitor);//trinco para enviar mensagem que entrou no bar 
            int buffer_len = snprintf(buffer, BUFFER_SIZE, "%i*19",id);//entra na sala de bilhar
            int n = write(data_socket, buffer, buffer_len);
            sleep(1);
            pthread_mutex_unlock(&mensagensMonitor);
            pthread_mutex_lock(&trincoVarGlobais);
            ...
            buffer_len = snprintf(buffer, BUFFER_SIZE, "%i*26",id);//ocorreu um jogo
            n = write(data_socket, buffer, buffer_len);
            sleep(1);
            pthread_mutex_unlock(&mensagensMonitor);
            pthread_mutex_lock(&trincoVarGlobais);
            ...
            buffer_len = snprintf(buffer, BUFFER_SIZE, "%i*20",id);//sai da sala de bilhar
            n = write(data_socket, buffer, buffer_len);
            sleep(1);
            pthread_mutex_unlock(&mensagensMonitor);
        ...
        if(pthread_create(&thread, &attr, &criarClientes, &id)!= 0)
         {
                ...
                {   
                    pthread_mutex_lock(&mensagensMonitor);
                    int buffer_len = snprintf(buffer,BUFFER_SIZE, "%i*0",id);
                    ret = write(data_socket, buffer, buffer_len);
                    sleep(1);
                    pthread_mutex_unlock(&mensagensMonitor);
                    if (ret == -1)
                    {
